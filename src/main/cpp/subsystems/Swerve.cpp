...
class Swerve : public TunerSwerveDriveTrain, public Subsystem, public SelfChecked
{
private:
  // faults and the table containing them
  unordered_set<Fault> _faults;
  nt::NetworkTableInstance inst = nt::NetworkTableInstance::GetDefault();
  FaultsTable _faultsTable =
      new FaultsTable(
  ...
  // teleop requests
  const RobotCentric _robotCentricRequest = new RobotCentric();
  const FieldCentric _fieldCentricRequest = new FieldCentric();

  const SwerveDriveBrake _brakeRequest = new SwerveDriveBrake();

  // auton request
private
  const ApplyRobotSpeeds _robotSpeedsRequest = new ApplyRobotSpeeds();

  // sysid requests
private
  const SysIdSwerveTranslation _translationSysIdRequest = new SysIdSwerveTranslation();
private
  const SysIdSwerveSteerGains _steerSysIdRequest = new SysIdSwerveSteerGains();
private
  const SysIdSwerveRotation _rotationSysIdRequest =
  ...
  // motors.
private
  const SysIdRoutine _sysIdRoutineTranslation =
      new SysIdRoutine(
...
private
  const SysIdRoutine _sysIdRoutineSteer =
      new SysIdRoutine(
          new SysIdRoutine.Config(
              null,        // Use default ramp rate (1 V/s)
              Volts.of(7), // Use dynamic voltage of 7 V
              null,        // Use default timeout (10 s)
              state->SignalLogger.writeString("Swerve SysId Steer Routine State", state.toString())),
          new SysIdRoutine.Mechanism(
              volts->setControl(_steerSysIdRequest.withVolts(volts)), null, this));

  // SysId routine for characterizing rotation.
private
  const SysIdRoutine _sysIdRoutineRotation =
      new SysIdRoutine(
          new SysIdRoutine.Config(
...
private
  double _lastSimTime = 0;
private
  Notifier _simNotifier;

  @Logged(name = "Driver Chassis Speeds") private const ChassisSpeeds _driverChassisSpeeds = new ChassisSpeeds();

  @Logged(name = "Is Field Oriented") private boolean _isFieldOriented = true;

  @Logged(name = "Is Open Loop") private boolean _isOpenLoop = true;

  @Logged(name = "Ignore Vision Estimates") private boolean _ignoreVisionEstimates = false;

  @Logged(name = VisionConstants.blueArducamName) private const VisionPoseEstimator _blueArducam =
      VisionPoseEstimator.buildFromConstants(VisionConstants.blueArducam);

  // for easy iteration with multiple cameras
private
  const List<VisionPoseEstimator> _cameras = List.of(_blueArducam);

private
  const List<VisionPoseEstimate> _acceptedEstimates = new ArrayList<>();
private
  const List<VisionPoseEstimate> _rejectedEstimates = new ArrayList<>();

private
  const Set<Pose3d> _detectedTags = new HashSet<>();

private
  const VisionSystemSim _visionSystemSim;
...
public
   ...
   * command in null.
   */
  @Logged(name = "Current Command") public const String currentCommandName()
  {
...
private
  const void addFault(String description, FaultType faultType)
  {
...
public
  const boolean hasFault(String description, FaultType faultType)
  {
...
private
  void startSimThread()
  {
    ...
    _simNotifier =
        new Notifier(
            ()->{
              const double currentTime = Utils.getCurrentTimeSeconds();
...
public
   ...
   */
public
  Command drive(InputStream velX, InputStream velY, InputStream velOmega)
  {
    return run(()->{
   ...
   */
public
  void drive(double velX, double velY, double velOmega)
  {
...
public
  Rotation2d getHeadingAtTime(double timestamp)
  {
...
private
  void updateVisionPoseEstimates()
  {
    ...
    for (VisionPoseEstimator cam : _cameras)
    {
      ...
      estimates.forEach(
          (estimate)->{
            ...
            if (estimate.isValid())
              _acceptedEstimates.add(estimate);
            else
              _rejectedEstimates.add(estimate);
          });
    }
  }
...
private
  Command selfCheckModule(String name, SwerveModule module)
  {
    return shiftSequence();
  }

  @Override public Command selfCheck()
  {
    return shiftSequence(
